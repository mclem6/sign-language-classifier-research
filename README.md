...
Lessons Learned 
One important detail for transfer learning is that you must preprocess your data appropriately for each pretrained model. You must then apply the per-model preprocessor function to transform the image into a structure more similar to the one the model was trained on. When I did not apply the per-model preprocessor, I was only achieving ~.4 accuracy, which is only slightly better than the model guessing!
